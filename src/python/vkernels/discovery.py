...
public headers under ``src/c/vkernels/kernels/`` (kernels) and
``src/c/vkernels/comm/`` (communication primitives) and pairing each declared
...
def resolve_root(override: str | None = None) -> Path:
    ...
    if not (root / "src" / "c" / "vkernels").is_dir():
        raise FileNotFoundError(
