// vkernels/capi/capi.cpp — C ABI core: error plumbing, version/config,
// device/stream handles and the comm entry points declared in capi.hpp.
//
// The kernel entry points live in the sibling domain TUs
// (capi_kernels.cpp, capi_attn.cpp, capi_moe.cpp); the shared error
// plumbing and the exception-to-status translation macros live in
// capi_internal.hpp. Exceptions (the VK_EXPECTS / VK_ENSURES contract
// checks, std::bad_alloc) cannot cross the C ABI, so every exported
// function wraps the C++ API in a try/catch: they are translated into a
// status code plus a thread-local message readable via vk_last_error() /
...
// failing call on the same thread. Internal linkage — the only access is
// through the accessors below (also used by the VK_CAPI_CATCH macros).
static thread_local std::string g_last_error;

// Status code of the most recent failing call (VK_OK when none).
static thread_local int g_last_error_code = VK_OK;
...
int last_error_code() { return g_last_error_code; }
...
const char* vk_last_error(void) { return vkernels::capi::last_error(); }

int vk_last_error_code(void) { return vkernels::capi::last_error_code(); }

void vk_set_last_error(int code, const char* message) {
  vkernels::capi::set_last_error(code, message);
}
