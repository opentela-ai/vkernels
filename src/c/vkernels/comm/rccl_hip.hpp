...
// because the HIP entry points take `ncclComm_t` / `hipStream_t`, which must
...
struct ncclComm;
typedef struct ncclComm* rcclComm_t_hip;  // mirrors rccl.h's ncclComm_t
struct ihipStream_t;
...
// ---------------------------------------------------------------------------
// RcclComm — RAII over an ncclComm_t
// ---------------------------------------------------------------------------
//
...
// `ncclCommInitAll` when every rank shares one process, or per-rank
// `ncclCommInitRank` from a `ncclUniqueId` for the cross-process/multi-node
...
//
// Lifetime: the comm is destroyed (`ncclCommDestroy`) in ~RcclComm, so
...
// Throws std::runtime_error on any ncclCommInitAll failure (the ncclResult_t
...
// ncclGetUniqueId on rank 0 and broadcast to every rank). `rank`/`world`
// identify this process; the other ranks call this with the same id. Throws
// std::runtime_error on any ncclCommInitRank failure.
...
// std::runtime_error on ncclGetUniqueId failure.
std::string rccl_unique_id();

// ---------------------------------------------------------------------------
// RcclChannel — Channel over ncclSend / ncclRecv
// ---------------------------------------------------------------------------
//
...
// per-channel device staging buffer, issues ncclSend/ncclRecv on the
...
// path is RcclAllreducePlanHip below (one ncclAllReduce, no ring).
...
// ---------------------------------------------------------------------------
// RcclAllreducePlanHip — graph-capturable ncclAllReduce (issue #10)
// ---------------------------------------------------------------------------
//
// Mirrors the host RcclAllreducePlan API (validate once at construction;
// execute() enqueues exactly ONE ncclAllReduce on the caller's stream) so the
// same test/bench harness drives both. The graph-capturable contract: one
// ncclAllReduce == one hipGraph node, so a multi-layer pipeline can capture
...
// graph contains a single ncclAllReduce node. execute() itself never begins
