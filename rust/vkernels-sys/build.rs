...
//! crate together with the C ABI from csrc/vkernels/capi/capi.cpp (which is
...
fn main() {
    ...
    // rust/vkernels-sys -> repository root.
    ...
    // add_subdirectory(csrc) places the archive in <build>/csrc for the
    // Makefiles and Ninja generators; advertise both candidates.
    let build_dir = dst.join("build");
    println!(
        "cargo:rustc-link-search=native={}",
        build_dir.join("csrc").display()
    );
    println!("cargo:rustc-link-search=native={}", build_dir.display());
    println!("cargo:rustc-link-lib=static=vkernels");

    // Sanity check: the C ABI (csrc/vkernels/capi/capi.cpp) must be compiled
...
fn verify_capi_symbols(build_dir: &Path) {
    let archive = build_dir.join("csrc").join("libvkernels.a");
    ...
    if !symbols.lines().any(|l| l.contains(" T vk_version")) {
        panic!(
            "the built libvkernels.a does not export the C ABI (vk_version); \
             make sure csrc/CMakeLists.txt compiles csrc/vkernels/capi/capi.cpp"
        );
    }
}
