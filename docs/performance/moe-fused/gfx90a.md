...
|---:|---:|---:|---:|---:|---:|
| 128  | 0.958 | 3362 | 2.065 | 1559 | 0.46× |
| 256  | 1.414 | 4557 | 2.076 | 3105 | 0.68× |
| 512  | 2.688 | 4794 | 2.368 | 5442 | 1.14× |
| 1024 | 5.166 | 4988 | 4.098 | 6286 | 1.26× |
| 2048 | 10.231 | 5038 | 8.752 | 5888 | 1.17× |

The prefill config wins once each expert fills its 64-row block (M ≥ 512);
below that the 64-row padding waste dominates and decode is faster. Even
the prefill config reaches only ~3% of per-GCD peak — the kernel is
latency-bound (see Caveats).
...
- **Occupancy-bound, not barrier-bound.** LDS double-buffering (2-stage
  ...
  removed (they write different LDS buffers) — a free, small win. The lever
  is *more* occupancy (fewer VGPRs / less LDS), not deeper pipelining.
...
CPU oracle, Python bindings, and a prefill (BM=64/BN=128) tile config.
